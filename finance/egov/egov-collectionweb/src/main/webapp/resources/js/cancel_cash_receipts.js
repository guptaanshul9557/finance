...
$(document).ready(function(){
	...
	function getFormData($form) {
		...
		if (indexed_array['receiptDate']) {
					var receiptTimestamp = convertDateToLongTimestamp(indexed_array['receiptDate']);
					if (receiptTimestamp) {
						indexed_array['receiptDate'] = receiptTimestamp;
					}
				}
...
jQuery('#btnsearch').click(function(e) {
	var heading = "Remitted Cash List";
	$("#surrenderCashHeading").html(heading);
	callAjaxSearch();
});
...
function convertDateToLongTimestamp(dateString) {
...
function callAjaxSearch() {
	...
	var receiptNumber = $("#receiptNumberId").val();
	var receiptDate = $("#receiptDateId").val();
	if(instrumentMode == ""){
		bootbox.alert(instrumentModeMendatoryMessage);
		return false;
	}else if(receiptNumber == ""){
		bootbox.alert(receiptNumberMandatoryMessage);
		return false;
	}else if(!receiptDate){
		bootbox.alert(receiptDateMandatoryMessage);
		return false;
	}

	var fileName = 'Remitted Cash List';
	...
	var heading = "Remitted Cash List";
							reportdatatable = drillDowntableContainer.DataTable({
								ajax : {
									url : '/services/collection/cancel/cash_receipt/_search',
									type : "get",
