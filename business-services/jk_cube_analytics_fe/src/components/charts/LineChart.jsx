...
const LineChart = ({
  ...
  // Add default colors to datasets if not provided
  const enhancedDatasets = (finalDatasets || []).map((dataset, index) => ({
    ...
    backgroundColor: dataset.backgroundColor || `${globalConfig.colors.chartColors[index % globalConfig.colors.chartColors.length]}33`,
  ...
  const options = {
    ...
    onClick: (event, elements) => {
      ...
      if (elements.length > 0 && onPointClick && event.native?.type === 'click') {
        const index = elements[0].index;
        const label = chartData.labels[index];
        const value = chartData.datasets[0].data[index];
        onPointClick({ name: label, value: value, index: index });
      }
    },
  };
